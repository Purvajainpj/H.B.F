...
#     st.sidebar.markdown("**Recommended Prompts**")
#     prompts = [
...
#     st.sidebar.markdown("**Recommended Prompts**")
#     prompts = [
#         "What is the merchant network for gift card?",
#         "Expenses for how many days are covered in pre-hospitalization?",
#         "Can employee choose the value of the gift card every year?"
#     ]
#     for prompt in prompts:
#         if st.sidebar.button(prompt):
#             st.session_state.user_input = prompt
...
#     st.sidebar.markdown('<div style="background-color: #ff6347; padding: 10px; border-radius: 5px; color: white; font-weight: bold;">General</div>', unsafe_allow_html=True)
#     st.sidebar.markdown("**Recommended Prompts**")
#     st.sidebar.markdown("No specific prompts available for this role.")
...
If the user's query is out of scope or unrelated to the selected {Role}, then also strictly provide follow-up questions based on the documents or content specific to the {Role} that could help guide the user back to relevant topics within your designated role.

For Example:

Question: What account has a $2,100 credit balance as of june 30th?
Response: I apologize, but as an HR assistant, I do not have access to financial information such as specific account balances. My role is to provide guidance on HR best practices, employment laws, and company policies. If you have any HR-related questions, I would be happy to assist you.
Follow-up Questions:
1. What is the merchant network for gift card?
2. Expenses for how many days are covered in pre-hospitalization?
3. can employee choose the value of the gift card every year?

Also give follow-up questions in case of history enabled.
"""
...
#     # Sidebar for follow-up questions
#     with st.sidebar:
...
#         # Split the full response to separate the answer and follow-up questions
#         parts = full_response.split("Follow-up Questions:")
...
#         # Display the answer in the chat message
#         message_placeholder.markdown(answer)

#         # Append the answer (without follow-up questions) to the session state
#         st.session_state.messages.append({"role": "assistant", "content": answer})
#         print(full_response)

#     # Update follow-up questions based on the response
#     follow_up_questions = get_follow_up_questions(full_response)
...
# # Sidebar with recommended prompts
# st.sidebar.markdown("   ")
# st.sidebar.markdown("**Recommended Prompts**")
...
# for prompt in prompts:
#     st.sidebar.button(prompt, on_click=set_user_input, args=(prompt,))
...
# # Use the user input or the stored input from button clicks
# if user_input:
#     st.session_state.user_input = user_input

# if st.session_state.user_input:
#     # Display user message in chat message container
#     st.chat_message("user", avatar="🧑‍💻").markdown(st.session_state.user_input)
#     st.session_state.messages.append({"role": "user", "content": st.session_state.user_input})
    
#     user_query = st.session_state.user_input + dropdown_3_prompt
#     if use_memory:
#         response = generate_response_with_memory(user_query=user_query, stream=True)
#     else:
#         response = generate_response_without_memory(user_query=user_query, stream=True)
...
# import streamlit as st
# import openai  # Assuming you're using OpenAI for generating responses
...
# Function to get follow-up questions
def get_follow_up_questions(full_response):
    #parts = full_response.split("Follow-up Questions:" or "Follow-up questions:")
    parts = re.split(r"Follow-up [Qq]uestions:", full_response, maxsplit=1)
    if len(parts) > 1:
        questions_part = parts[1].strip()
        return [q.strip() for q in questions_part.split('\n') if q.strip()]
    return []

# Sidebar with recommended prompts
st.sidebar.markdown("   ")
st.sidebar.markdown("**Recommended Prompt**")

# Define ROLE (you should set this based on your application's logic)
#ROLE = "HR"  # Example role

if ROLE == "Product_Manager":
    prompts = [
        "What were some of the common photography-related problems with smartphones mentioned in the discussion?"
    ]
elif ROLE == "HR":
    prompts = [
        "What is the merchant network for gift card?"
    ...
    prompts = [
        "Who prepares money reminder when the products are sold in real cash?"
    ]
elif ROLE == "Accountant":
    prompts = [
        "What account has a $2,100 credit balance as of June 30th?"
    ]
else:
    prompts = []
    st.sidebar.markdown("No specific prompts available for this role.")
...
for question in st.session_state.follow_up_questions:
    st.sidebar.button(question, on_click=set_user_input, args=(question,))
...
if st.session_state.user_input:
    ...
    # Add your logic here to generate the dropdown_3_prompt
    dropdown_3_prompt = ""  # This should be defined based on your application's logic
    
    user_query = st.session_state.user_input + dropdown_3_prompt
    
    # Define these functions based on your implementation
    use_memory = False  # Set this based on your application's logic
    if use_memory:
        response = generate_response_with_memory(user_query=user_query, stream=True)
    else:
        response = generate_response_without_memory(user_query=user_query, stream=True)
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        # Split the full response to separate the answer and follow-up questions
        #parts = full_response.split(r"Follow-up [Qq]uestions:", full_response, maxsplit=1)
        parts = re.split(r"Follow-up [Qq]uestions:", full_response, maxsplit=1)
        # Keep only the answer part
        answer = parts[0].strip()
        # Replace dollar signs with escaped dollar signs
    ...
    # Update follow-up questions based on the response
    st.session_state.follow_up_questions = get_follow_up_questions(full_response)
